...
function setup() {
  ...
  divA = createDiv().position(650, 50).class('flex-container');
 
  let divA1 = createDiv().parent(divA).class('subdiv');
  let spanr = createSpan("r: ").parent(divA1);
  rSlider = createSlider(100, 400, 250).style('width', '180px').parent(divA1).class('mySliders');
  ...
  let divB = createDiv().position(650, 350).class("flex-container");
  ...
  divC = createDiv().position(50, 700).class('colors');
  colorP = createP('Choose colors:').position(50, 630);
