...
function setup() {
  ...
  // Controls for shader
  divA = createDiv().position(350, 730).class('flex-container');
  let para3 = createP('Shader').parent(divA);
  sel1 = createSelect().parent(divA).class('dropdown');
  ...
  let para2 = createP('Combining Shapes').parent(divB);
  let spanr = createSpan("r: ").parent(divB);
  rSlider = createSlider(1, 10, 5).style('width', '180px').parent(divB);
  
  let spana = createSpan("mix: ").parent(divB);
  mixSlider = createSlider(0, 10, 1, 0.1).style('width', '180px').parent(divB);
  
  // For star shape
  let spanb = createSpan("starr: ").parent(divB);
  rsSlider = createSlider(0, 2, 1, 0.1).style('width', '180px').parent(divB);
  
  let spann1 = createSpan("n1: ").parent(divB);
  nSlider = createSlider(0, 20, 8).style('width', '180px').parent(divB);
  
  let spann2= createSpan("m: ").parent(divB);
  mSlider = createSlider(0, 20, 6).style('width', '180px').parent(divB);
